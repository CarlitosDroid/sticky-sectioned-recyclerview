...
public class SectioningAdapter extends RecyclerView.Adapter<SectioningAdapter.ViewHolder> {
	...
	private static class GhostHeaderViewHolder extends ViewHolder {
		GhostHeaderViewHolder(View itemView) {
			super(itemView);
		}
     ...
     * Called when a ViewHolder is needed for a section item view
     *
     * @param parent The ViewGroup into which the new View will added after it is bound to an adapter position
     * @return A new UserViewHolder holding an user view
     */
	 ...
	 */
    private GhostHeaderViewHolder onCreateGhostHeaderViewHolder(ViewGroup parent) {
    ...
    /**
     *
     * Called to display user data at particular position
     *
     * @param viewHolder the view holder to update
     * @param sectionIndex the index of the section containing the user to update
     */
    public void onBindUserViewHolder(UserViewHolder viewHolder, int sectionIndex) {
    }
	 ...
	 */
    private void onBindGhostHeaderViewHolder(GhostHeaderViewHolder viewHolder, int sectionIndex) {
	}
	 ...
	 */
    private void onBindFooterViewHolder(FooterViewHolder viewHolder, int sectionIndex) {
	}
	...
	public int getAdapterPositionForSectionHeader(int sectionIndex) {
		if (doesSectionHaveHeader(sectionIndex)) {
			...
			throw new InvalidParameterException("MySectionEntity " + sectionIndex + " has no header");
		}
	}
	...
	public int getAdapterPositionForSectionFooter(int sectionIndex) {
		if (doesSectionHaveFooter(sectionIndex)) {
			...
			throw new InvalidParameterException("MySectionEntity " + sectionIndex + " has no footer");
		}
	}
	 ...
	 */
    private void notifyAllSectionsDataSetChanged() {
		buildSectionIndex();
	...
	public int getItemViewType(int adapterPosition) {
		...
		if (section.hasHeader && section.hasFooter) {
            ...
            if(adapterPosition==0){ //in our case the first position is UserViewHolder but you can create doesUserViewType method
                return TYPE_USER_PROFILE;
            }else{
                return TYPE_ITEM;
            }
	...
	public void onBindViewHolder(ViewHolder holder, int adapterPosition) {
		...
		switch (holder.getItemViewType()) {
            ...
            case TYPE_USER_PROFILE:
                onBindUserViewHolder((UserViewHolder) holder, section);
                break;
